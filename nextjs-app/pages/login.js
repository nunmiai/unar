...
export default function LoginPage() {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      const res = await fetch(`${AUTH_API}/register`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: signupForm.name, email: signupForm.email, phone: signupForm.phone, password: signupForm.password }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  const handleVerify = async (e) => {
    ...
    try {
      const res = await fetch(`${AUTH_API}/verify`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: pendingEmail, code: verifyForm.code }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  const handleGoogleLogin = () => {
    window.location.href = `${AUTH_API}/google-auth`;
  };
